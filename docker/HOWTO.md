...
## 1.1 최신 기능 요약 (2026-08-29)

| 영역 | 기능 |
|------|------|
| 테넌트 LLM | 관리자 `/admin/projects` → "⚙ LLM 설정"에서 프로젝트별 Gemini 키/모델 지정(테스트 후 적용), OpenRouter는 전역 `.env` 전용 |
| 필수 메뉴 | 모든 프로젝트에 **"AI비서란?"** 메뉴 마지막 배치 — DB 공통 답변, 편집/삭제 불가 |
| 위젯 | AI 로고 런처, 음성 입력(두 줄 버튼), 답변 생성 중 입력 잠금 |
| 프로젝트 수정 | 이름/URL 변경 금지, 도메인 유형·위젯 테마만 변경 가능 |
| 로그아웃 | 세션 확실 삭제(CSRF 우회 + 쿠키 삭제) |
| SEO | 전 페이지 noindex·nofollow, `robots.txt` 전체 접근 금지, `llms.txt` 제공 |
| 이용약관 | 프로젝트 하단 "읽어볼 내용" 아코디언(이용약관/AI 이용고지/개인정보처리방침/프로그램 사용동의) |

> **CSRF 오리진**: `127.0.0.1:8080` 접속 시 로그인 403이 나면 `docker-compose.yml`의
> `CSRF_TRUSTED_ORIGINS`에 `http://127.0.0.1:8080`이 포함되어 있는지 확인하세요.
