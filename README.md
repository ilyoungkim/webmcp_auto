...
| 영역 | 기능 |
|------|------|
| 계정 | 가입·로그인·비밀번호 변경(강제변경 포함), 플랜(free/pro/admin) |
| 프로젝트 | URL 입력 → 크롤 → LLM Q&A → 위젯 생성 자동화, **최대 5개** 생성 |
| 프로젝트 수정 | **이름/URL 변경 금지**, 도메인 유형·위젯 테마만 변경 가능 |
| 빠른메뉴 | 질문 편집 **1회 제한**, 저장된 소스로 답변 재생성 |
| 위젯 | 5종 테마, 미리보기, `bundle.zip` 설치 번들(난독화) |
...
| 이용약관 | 프로젝트 페이지 하단 아코디언(사용자권리/LLM 주의사항/개인정보보호/프로그램 사용동의) |
