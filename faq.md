...
### Q. 빠른메뉴 질문 편집 후 "AI비서란?"이 사라져요 (4개만 생성)

**원인**: `project_menus_regenerate`가 `menus`를 `is_required=False`로만 필터링해 필수 메뉴가 재생성 대상에서 빠짐.

**해결**: `menus` 필터에서 `is_required=False` 제거 → 필수 메뉴 포함. `regenerate_qna`가 is_required면 DB 공통 답변 사용.

### Q. 연락처 답변에서 전화번호·예약 정보가 빠져요

**원인**: `_remove_empty_email_section`이 유효 이메일 없을 때 '이메일' 섹션 시작 후 다음 섹션 마커까지 통째로 제거해 전화번호·예약·주소가 함께 잘림.

**해결**: '이메일' 단어가 있는 줄만 제거하고, 전화번호·예약·상담·주소·카카오·채널 등 실질 정보 줄은 보존.

### Q. 사용자가 빠른메뉴를 이미 편집해서 더 이상 수정이 안 돼요

**원인**: 빠른메뉴 질문 편집은 1회만 허용(`menus_edited=True`).

**해결**: admin@local로 로그인 → Manage Projects → 해당 프로젝트의 **"빠른메뉴 편집 초기화"** 버튼 클릭 → `menus_edited=False`로 복원되어 사용자가 다시 편집 가능.

### Q. 회원가입 시 "가입 코드"를 요구해요

**원인**: 회원가입에 가입 코드(`SignupCode`) 필수 입력이 추가됨.

**해결**: 기본 코드 **`OneAir1234`** 입력 (ko/en 양쪽 사일로 허용). 관리자는 DB에 사일로별 코드 추가 가능 (`lang`: ''=양쪽, 'ko', 'en').
